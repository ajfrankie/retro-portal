...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{!! route('admin.cake.update', $cake->id) !!}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <div class="mb-3">
                                    <label for="formrow-name-input" class="form-label">@lang('translation.name')</label>
                            ...
                            <div class="col-md-3">
                                <div class="mb-3">
                                    ...
                                    <select class="form-control @error('veg_nonveg') is-invalid @enderror"
                                        name="veg_nonveg" id="veg_nonveg">
                                        <option value="">@lang('translation.select-cake-type')</option>
                                        <option value="veg" {{ old('veg_nonveg', $cake->veg_nonveg ?? '') == 'veg' ? 'selected' : '' }}>Veg</option>
                                        <option value="non-veg" {{ old('veg_nonveg', $cake->veg_nonveg ?? '') == 'non-veg' ? 'selected' : '' }}>Non-Veg</option>
                                    </select>
                                    @error('veg_nonveg')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                    ...
                                    <select class="form-select @error('category_id') is-invalid @enderror"
                                        ...
                                        @foreach ($categories as $category)
                                            <option value="{{ $category->id }}"
                                                {{ old('category_id', $cake->category_id ?? '') == $category->id ? 'selected' : '' }}>
                                                {{ $category->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-3">
                                <div class="mb-3">
                                    ...
                                    <select class="form-control @error('availability') is-invalid @enderror"
                                        ...
                                        <option value="in-stock" {{ old('availability', $cake->availability ?? '') == 'in-stock' ? 'selected' : '' }}>In Stock</option>
                                        <option value="out-of-stock" {{ old('availability', $cake->availability ?? '') == 'out-of-stock' ? 'selected' : '' }}>Out of Stock</option>
                                        <option value="pre-order" {{ old('availability', $cake->availability ?? '') == 'pre-order' ? 'selected' : '' }}>Pre-Order</option>
                                    </select>
                                    @error('availability')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                            ...
                            <div class="col-md-12">
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control @error('description') is-invalid @enderror"
                                        name="description" id="formrow-description-input"
                                        placeholder="@lang('translation.enter-description')" rows="4">{{ old('description', $cake->description ?? '') }}</textarea>
                                    @error('description')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
